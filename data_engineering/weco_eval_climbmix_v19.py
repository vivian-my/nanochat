"""WeCo eval v19: Modal-based d8 selection eval (v19 (gpt-5.5, full 9-feature bank, ablation priors)).

Each iteration:
  1. Load `select_docs` from data_engineering/data_select_climbmix_v19
...
run_prefix = os.environ.get("WECO_RUN_PREFIX", "climbmix_v19")
...
def log(msg):
    print(f"[weco_eval_v19] {msg}", flush=True)


def validate_selection(idx, budget):
    from data_engineering.data_select_climbmix_v19 import N_TOTAL
...
def main():
    ...
    from data_engineering.data_select_climbmix_v19 import select_docs, BUDGET
    t0 = time.time()
