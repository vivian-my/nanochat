# WECO ClimbMix v16 — data-selection instructions (cheap features, 100-shard pool, ratio=10.5)

## Goal

Write `select_docs(budget, seed)` to pick **880,000** documents from an
**8,460,288-doc pool** that minimize the mean `val_bpb` of a d8 model trained
on the selected set.

The trainer averages **4 independent seeds** (~440 M tokens each, d8 FP8,
...
Budget: 880,000 / 8,460,288 = **10.4 % keep rate**.
