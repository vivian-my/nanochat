"""WeCo eval v17: Modal-based d8 selection eval (v17 (gpt-5.5, full 9-feature bank, ablation priors)).

Each iteration:
  1. Load `select_docs` from data_engineering/data_select_climbmix_v17
  2. Compute selection (BUDGET=879,999 indices)
...
Requires the Modal app to be deployed first:
  /home/nvidia/yan/modal_env/bin/modal deploy \\
      /home/nvidia/yan/nanochat/data_engineering/modal_train_d8_lexical.py
"""
import os
import sys
import time
...
run_prefix = os.environ.get("WECO_RUN_PREFIX", "climbmix_v17")
...
def log(msg):
    print(f"[weco_eval_v17] {msg}", flush=True)


def validate_selection(idx, budget):
    from data_engineering.data_select_climbmix_v17 import N_TOTAL
...
def main():
    ...
    from data_engineering.data_select_climbmix_v17 import select_docs, BUDGET
    t0 = time.time()
    ...
    # Reference the deployed Modal function (no app.run() needed since deployed).
    train_d8 = modal.Function.from_name("nanochat-d8-lexical-eval", "train_d8")
